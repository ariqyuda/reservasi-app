...
func (auth *AuthRepo) Register(email, nama, password, nik, gender, tgl_lahir, tmpt_lahir, alamat, no_hp string) error {
	...
	_, err = auth.db.Exec(sqlStmt, idUser, nik, nama, gender, tgl_lahir, tmpt_lahir, alamat, no_hp, time.Now())
	if err != nil {
		return err
	}
