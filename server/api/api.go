...
func NewAPI(usersRepo user.UserRepo, authRepo auth.AuthRepo, adminRepo admin.AdminRepo,
	...
	mux.Handle("/api/peugas/lihat/poli/dokter", api.GET(api.AuthMiddleWare(api.PetugasMiddleware(http.HandlerFunc(api.lihatDokter)))))
	mux.Handle("/api/petugas/lihat/poli/dokter/jadwal", api.GET(api.AuthMiddleWare(api.PetugasMiddleware(http.HandlerFunc(api.fetchJadwalDokter)))))
